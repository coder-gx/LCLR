 ...
 myCompiler::myCompiler(QWidget* parent) : QMainWindow(parent)
{
	...
	//导入代码或者文法
	connect(this->ui->action_Code, &QAction::triggered, this, &myCompiler::loadCode);//记得在输出窗口print结果,状态转换
	connect(this->ui->action_Grammar, &QAction::triggered, this, &myCompiler::loadGrammar);//写到my_grammar.txt里
	...
	connect(this->ui->action_Restart, &QAction::triggered, this, &myCompiler::restart);//写到my_grammar.txt里 
...
int myCompiler::readGrammar()
{
	...
	tip = "文法已读入，可在results/Tables.csv下查看：Action Goto表\n";
	this->ui->textBrowser->append(tip);
	this->grammarFlag = 1;
	return 1;
}
